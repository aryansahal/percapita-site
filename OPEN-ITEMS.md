...
| 3b | **The commitment photo is no longer on the page**, but the file is still in use. Our Commitment is now a plain band and `PHOTOS.quote` was removed; `public/photos/commitment.jpg` is still referenced by the structured data as the business `image`, which is what search results can show. **Do not delete the file.** | `src/lib/seo.ts` line 92 | Nothing, unless the removal was unintended - in which case restore the photo band from git. |
