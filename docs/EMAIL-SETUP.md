# Setting up email for the contact form
...
| | Value | Meaning |
|---|---|---|
| MX | `mx.zoho.com`, `mx2`, `mx3` | Mail is already on Zoho, `.com` data centre (not `.in`) |
| TXT on `percapita.in` | **none** | No SPF record |
| `_dmarc.percapita.in` | **none** | No DMARC record |
| A | `190.92.174.104`, LiteSpeed | cPanel shared host, serving a placeholder page |

So the mailbox side is done and the DNS side is untouched. Steps 1 and 2 make
the form work; step 3 is what keeps the mail out of spam folders.
...
2. Use selector `zoho`. Zoho prints a long public key.
3. Add it as TXT at host `zoho._domainkey`.
...
nslookup -type=TXT zoho._domainkey.percapita.in
```
