...
| 3c | **The hero photo is under-resolution.** Client-supplied 25 Sep 2026 at 692x640. The slot renders 536x640, which is 1072x1280 on a high-DPI screen, so it displays at roughly twice its real resolution and will look soft. Framing is set with `objectPosition="70% 50%"` because centring clipped a face. | `public/photos/hero.jpg`, `Hero.tsx` | The same frame at 1400x1750 or larger. Ideally shot 4:5 so nothing has to be cropped. |
...
| 4a | **The /policy page is drafted, not approved.** Written 23 Sep 2026 from what the site verifiably does plus the obligations on an AMFI-registered distributor. **No lawyer or compliance professional has seen it.** It makes commitments on Percapita's behalf about data retention, grievance timelines and conflicts of interest. | `src/lib/policy.ts` | Legal and compliance sign-off before launch. Confirm in particular: the legal entity name ("Percapita Services"), the 21-day grievance response commitment, the five-year retention floor, and whether a named grievance officer is required. |
...
| 5 | **The store links go to a third-party-branded app.** Both listings are titled "Mint by Investwell", published by Excel Net Solutions - the same company behind percapita.investwell.app. The footer line naming the app was replaced with client-specified wording on 23 Sep 2026, so nothing on the page now tells a visitor where the tap lands. | `SiteFooter.tsx`, `clientLogin.ts` -> `MOBILE_APP` | Confirm Percapita is happy sending clients to an app branded as someone else, or ask InvestWell for a white-label build. |
